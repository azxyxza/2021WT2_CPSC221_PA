...
PixelPoint PriorityNeighbours::Remove() {
  // complete your implementation below
  PixelPoint removed = PixelPoint();
  double curr_min = removed.color.dist(refcolor);
  while (!IsEmpty()){
    for(unsigned long i = 0; i < points.size(); i++) {
      PixelPoint curr = points[i];
      double curr_dist = curr.color.dist(refcolor);
      if (curr_dist < curr_min){
        removed = curr;
      } else if (curr_dist == curr_min){
        if (curr.y < removed.y){
          removed = curr;
        } else if (curr.y == removed.y){
          if (curr.x < removed.x){
            removed = curr;
          }
        }
  ...
  return removed; // REPLACE THIS STUB
}
...
bool PriorityNeighbours::IsEmpty() const {
  // complete your implementation below
  return points.size() == 0; // REPLACE THIS STUB
}
